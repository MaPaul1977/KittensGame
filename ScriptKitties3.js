...
SK.Tasks = class {
    ...
    autoPraise(ticksPerCycle) {
        if (this.model.auto.praise && game.bld.get('temple').val > 0) {
            ...
            if ((currFaith > 600) || (currFaith >= 0.99 * maxFaith)) {
    ...
    autoCraft(ticksPerCycle) {
        ...
        if (this.model.auto.craft && game.workshopTab.visible) {
            for (const res of game.workshop.crafts) {
                ...
                // megalith|gear|alloy|concrete
                if (/megalith|gear|alloy|concrete|tanker|kerosene/.test(output)) { continue; }

                let craftCount = Infinity;
                let minimumReserve = Infinity;
                for (const input of inputs) {

                    if (output === 'steel' && input.name === 'iron' && craftCount > 0) { continue };
...
//                         console.log('here', input, res, inRes, inVal, outVal,
//                           craftCount, resourcePerCycle, resourcePerCycle / input.val, (inRes.maxValue - resourcePerCycle),
//                           inRes.value >= (inRes.maxValue - resourcePerCycle),
//                           resourcePerCycle >= inRes.maxValue);
...
//                         console.log('after', input, res, inRes, inVal, outVal,
//                           craftCount, resourcePerCycle, resourcePerCycle / input.val, (inRes.maxValue - resourcePerCycle),
//                           inRes.value >= (inRes.maxValue - resourcePerCycle),
//                           resourcePerCycle >= inRes.maxValue);
                    } else {
...
//                    if (res.name === 'steel') {
//                    console.log('rsrv', res, input, craftCount, minimumReserve, inVal * (this.model.option.minSecResRatio / 100) - outVal); }

                }

//                    if (res.name === 'steel') {     console.log('rsrv', res, craftCount, minimumReserve); }
    ...
    autoTrade(ticksPerCycle) {
        ...
        if (this.model.auto.trade) { // && game.diplomacyTab.visible) {
            ...
            if (goldResource.value > (goldResource.maxValue - goldPerCycle)) { // don't check catpower
                maxTrades = 0.6 * goldResource.value / 15;
            } else {
