...
'<button id="autoTrade" style="color:red" onclick="autoSwitch(autoCheck[3], 3, autoName[3], \'autoTrade\')"> Auto Trade </button> &nbsp;' +
'<input type="checkbox" id="elderCheck"><label for="elderCheck" onclick="autoSwitch(autoCheck[10], 10, autoName[10], \'elderCheck\')"><b>Elders</b></label><br>' + 
...
for (var i = 0; i < secondaryResources.length; i++) {
	...
	var resMath = priRes.value / secondaryResources[i][2];	
		if (resMath > 0 && gamePage.workshop.getCraft(secondaryResources[i][1]).unlocked && secondaryResources[i][1] == "eludium") {
			gamePage.craft(secondaryResources[i][1], resMath);
		} else if (resMath > 0 && secRes.value < (priRes.value * (secResRatio / 100)) && gamePage.workshop.getCraft(secondaryResources[i][1]).unlocked) {
			//console.log(secondaryResources[i][1], (resMath * (secResRatio / 100)));
